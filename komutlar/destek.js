...
const emoji = message.client.emojis.get('');
      ...
      let destekKanal = "732064633980059668";//Canlı desteğe cevap vereceğiniz kanal ID si
      ...
      if (reason === 'success') {
        ...
        client.on('message', message => {
          function contact() {
            ...
            if (message.channel.id === chan.id) client.channels.get(destekKanal).send(`:tickets: ${message.author.tag} : ${message.content}`)
            if (message.channel.id === destekKanal) chan.send(`:microphone2: ${message.author.tag} : ${message.content}`)
          }
          contact(client)
        })
      }
